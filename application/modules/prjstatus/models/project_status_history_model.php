...
class Project_status_history_model extends MY_Model {
	...
	public function get_all_history($project_id){
		...
		$this->db->join('project_contacts','project_contacts.id = project_status_histories.project_contact_id');
		$this->db->join('contacts','contacts.id = project_contacts.contact_id');
		$this->db->join('user_details','user_details.uacc_id_fk = project_status_histories.created_by');
		$this->db->join('grouptypes','grouptypes.id = project_contacts.type_id');
		$this->db->join('prjstatuses','prjstatuses.id = project_status_histories.prjstatus_id');
		$this->db->order_by('project_status_histories.created_at');
		return $this->db->get($this->_table)->result_array();
	}
	...
	public function get_all_history_user($project_id){
		...
		$this->db->join('grouptypes','grouptypes.id = project_contacts.type_id');
		$this->db->join('prjstatuses','prjstatuses.id = project_status_histories.prjstatus_id');
		$this->db->group_by('project_status_histories.created_by');
