	...
	<div class="col-lg-12">
		<div id="tab">
			<ul class="nav nav-tabs" role="tablist">
				...
				<?php ($active) ? $class ='class="active"' : $class =''; ?>
				<li><a id="<?php echo $type['id'] ?>" href="#tab-<?php echo $type['id'] ?>" role="tab" data-toggle="tab"><?php echo ucwords(strtolower($type['grouptype_desc'])); ?></a></li>
				<?php $active = false; ?>
				<?php endforeach; ?>
...
$(document).ready(function() {
	...
	$('a[data-toggle="tab"]').on('shown.bs.tab', function (e) {
		...
		var ex = '#data-table-'+group_id;
		draw_datatable(ex,group_id);
	})

	function draw_datatable(ex,group_id){
		$(ex).dataTable().fnDestroy();
		$(ex).dataTable({
			"processing": true,
			"serverSide": true,
			"ajax": {
				"url":domain + "/project/contacts/<?php echo $project['id']?>/"+group_id,
			},
			"language": {
			     "processing": "<img src='"+domain+"/assets/plugins/DataTables-1.10.2/images/processing.gif'><br><br>Processing please wait.."
			},
			"columns": [
				{ "data": "contact_name","width": "35%" },
				{ "data": "company" },
			],
		});
	}
          	...
          	$.ajax({
				...
				success: function(data){
					if(data.status == 'ok'){
						alert(data.message);
						$("#addModal").modal('hide');
						var ex = '#data-table-'+data.group_id;
						draw_datatable(ex,data.group_id);
